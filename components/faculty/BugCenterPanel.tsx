            ...
            {["All", "Open", "Investigating", "In Progress", "Resolved"].map(filter => {
              ...
              return (
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-colors border ${selectedFilter === filter ? 'bg-[#D0BCFF] text-[#2A1B4E] border-[#D0BCFF]' : 'bg-white/5 text-white/70 border-white/10 hover:bg-white/10'}`}
                >
                  {filter} ({count})
                </button>
              )
                          ...
                          {isVid ? (
                            ...
                            >
                              ...
                              <div className="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors">
                                <PlayCircle className="w-12 h-12 text-white drop-shadow-lg opacity-80 group-hover:opacity-100 transition-opacity" />
                              </div>
                            </div>
                          ) : isPdfDoc ? (
                            ...
                            >
                              <img 
                                src={attachmentUrl} 
                                alt="Evidence" 
                                className="max-h-64 object-contain bg-black/50 group-hover:opacity-80 transition-opacity" 
                                onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/400x200/111/fff?text=Image+Not+Found'; }}
                              />
                              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30">
                                <Maximize className="w-8 h-8 text-white drop-shadow-md"/>
                              </div>
                            </div>
                          ) : null}
                        </div>
