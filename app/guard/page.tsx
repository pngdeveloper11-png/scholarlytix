...
export default function GuardPortal() {
  ...
  const getGuardStorageKey = () => `guard_unlocked_${getActiveCollegeId() || 'mit_mumbai'}`;

  useEffect(() => {
    setCollegeName(getActiveCollegeName());
    // Check if session is already unlocked for this specific college in this browser tab
    if (sessionStorage.getItem(getGuardStorageKey()) === "true") {
      setIsUnlocked(true);
    }
  }, []);
  ...
  const handleUnlock = async () => {
    ...
    setIsVerifying(true);
    try {
      // Fetch the master Guard PIN from the tenant's app_config/gate_security
      const snap = await getDoc(tenantDoc("app_config", "gate_security"));
      const validPin = snap.exists() ? snap.data().pin : "123456"; // Default fallback if not set by Admin/HOD

      if (pinInput === validPin) {
        sessionStorage.setItem(getGuardStorageKey(), "true");
        setIsUnlocked(true);
      } else {
  ...
  const handleScan = async (text: string) => {
    ...
    try {
      ...
      const passRef = tenantDoc("gate_passes", passId);
      ...
      if (result === "USED" || result === "EXPIRED" || result === "INVALID") {
        ...
        // Fetch Student Photo for Physical Guard Verification from tenant vault
        if (result.studentId) {
          const studentDoc = await getDoc(tenantDoc("students_directory", result.studentId));
  ...
  const lockPortal = () => {
    sessionStorage.removeItem(getGuardStorageKey());
    setIsUnlocked(false);
    setPinInput("");
    setScanStatus('IDLE');
  };
    ...
    return (
      <main className="relative min-h-screen w-full flex flex-col items-center justify-center p-6 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#1f103b] via-[#0a0a0a] to-black text-white">
        ...
        <div className="z-10 flex flex-col items-center max-w-sm w-full bg-[#111]/80 backdrop-blur-2xl border border-white/10 p-8 rounded-[2rem] shadow-2xl">
          <div className="w-16 h-16 bg-[#D0BCFF]/20 border border-[#D0BCFF]/30 rounded-2xl flex items-center justify-center mb-4">
            <Lock className="w-8 h-8 text-[#D0BCFF]" />
          </div>
          <span className="px-3 py-1 rounded-full bg-white/10 border border-white/15 text-xs font-bold text-[#D0BCFF] mb-2">
            {collegeName}
          </span>
      ...
      <header className="w-full bg-[#111] border-b border-white/10 p-4 flex justify-between items-center z-50 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#D0BCFF] font-medium">{collegeName} • Gate Pass Scanner</p>
          </div>
        </div>
