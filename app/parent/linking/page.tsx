...
export default function ParentLinking() {
  ...
  const fetchLinkedChildren = async (email: string) => {
    try {
      const qSingle = query(tenantCol("students_directory"), where("linkedParentEmail", "==", email));
      const qArray = query(tenantCol("students_directory"), where("linkedParentEmails", "array-contains", email));

      const [snapSingle, snapArray] = await Promise.all([getDocs(qSingle), getDocs(qArray)]);
      const mergedMap = new Map<string, any>();

      snapSingle.docs.forEach(d => mergedMap.set(d.id, { id: d.id, ...d.data() }));
      snapArray.docs.forEach(d => mergedMap.set(d.id, { id: d.id, ...d.data() }));

      setChildren(Array.from(mergedMap.values()));
    } catch (e) {
  ...
  const handleLinkChild = async () => {
    ...
    try {
      ...
      // Find the target student inside the active college vault
      const q = query(
        tenantCol("students_directory"),
      ...
      if (snap.empty) {
        ...
        const existingEmails: string[] = Array.isArray(studentData.linkedParentEmails)
          ? studentData.linkedParentEmails
          : studentData.linkedParentEmail
          ? [studentData.linkedParentEmail]
          : [];

        if (existingEmails.includes(parentEmail)) {
          alert("Your account is already linked to this student.");
        } else if (existingEmails.length >= 2) {
          alert("This student is already linked to the maximum of 2 parent accounts.");
        } else {
          // Check if a request already exists in the active college vault
          const reqQ = query(
          ...
          if (!reqSnap.empty) {
            ...
            await addDoc(tenantCol("link_requests"), {
      ...
      alert("Linking failed. Check connection.");
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <main className="relative min-h-screen w-full flex flex-col items-center justify-center p-4 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#1f103b] via-[#0a0a0a] to-black text-white overflow-hidden">
      ...
      <button
        ...
        className="absolute top-8 left-8 p-3 rounded-full bg-white/10 hover:bg-white/20 transition backdrop-blur-md z-50"
      >
              ...
              {showLinkForm ? (
                <div className="p-6 bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[2rem] space-y-4">
                  ...
                  <select value={semester} onChange={e => setSemester(e.target.value)} className="w-full bg-black/20 border border-white/10 rounded-xl p-4 outline-none">
                    {["Semester 1", "Semester 2", "Semester 3", "Semester 4", "Semester 5", "Semester 6", "Semester 7", "Semester 8"].map(s => <option key={s} value={s} className="bg-[#111] text-white">{s}</option>)}
                  </select>
